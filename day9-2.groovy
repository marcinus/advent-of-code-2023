...
def solve(List<String> input) {
    input.sum { line ->
        ...
        def stack = new ArrayList<List<Long>>()
        stack.push(series)
        while(!stack.last().every { it == 0 } ) {
            def lastSeries = stack.last()
            def newSeries = (1..<(lastSeries.size())).collect { lastSeries[it]-lastSeries[it-1]}
            stack.add(newSeries)
        }
        stack.reverse().inject(0) { cumulative, list -> list[0]-cumulative }
    }
}

assert 2 == solve(sampleInput)
println solve(input)