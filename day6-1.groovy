...
def solveSingle(long time, long distance) {
    println time*time-4*distance
    println Math.sqrt(time*time-4*distance)
...
def solve(List<String> input) {
    def (times, distances) = parse(input)
    (0..<(times.size())).inject(1) { cumulative, i ->
        cumulative * solveSingle(times[i], distances[i])
    }
}
...
def parse(List<String> input) {
    ...
    def times = timesMatcher.group(1).split(' ').findAll { it.size() > 0 }.collect { it.trim() as long }
    def distances = distancesMatcher.group(1).split(' ').findAll { it.size() > 0 }.collect { it.trim() as long }
    [times, distances]
}

assert 288 == solve(sampleInput)
println solve(input)